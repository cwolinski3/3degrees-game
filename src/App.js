...
function App() {
  ...
  // --- Steal state ---
  // (If you had an advanced steal mechanism, it would be integrated here.)
  const [stealTurn, setStealTurn] = useState(null);

  // Determine active player. (If stealTurn is active, off-turn (Player 1) gets control; otherwise, by turn parity.)
  ...
  // --- Auto-transition: Strength Selection ---
  useEffect(() => {
  ...
  // --- Auto-transition: Weakness Selection ---
  useEffect(() => {
  ...
  // getQuestion: Return a random question from the current topic for the given degree.
  ...
  // handleSelectCategory: When a category is chosen, clear previous turn state and choose a random topic.
  ...
  // useEffect: When currentTopic or degree changes, fetch a new question (if available).
  useEffect(() => {
  ...
  const handleSubmitAnswer = (providedAnswer) => {
    ...
    if (activePlayer === "Player 1") {
      ...
      // AI simulates a 70% chance of a correct answer.
