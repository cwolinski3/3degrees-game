...
function App() {
  ...
  // --- handleStealCategory: For Player 1 (off-turn) to select a new category for steal attempt ---
  ...
  const handleSubmitAnswer = (providedAnswer) => {
    ...
    if (stealTurn) {
      ...
      if (isCorrect) {
        ...
        // For steal, bonus is determined solely by Player 1's weakness (+10 if category is a weakness).
        ...
        if (degree < 3) {
          ...
          clearTurnState();
          // Player 1 retains turn and can choose a new category.
        }
      } else {
    ...
    if (isCorrect) {
      ...
      let basePoints = degree * 10;
      if (currentQuestion.type === "fill") basePoints *= 2;
      // For normal turns, bonus: For Player 1, bonus = +10 if category is in weaknesses; for AI similarly.
      ...
      // If active player is AI, then offer a steal opportunity to Player 1 if the category is available.
      if (activePlayer === "AI Player") {
        ...
        if (player1Available.includes(selectedCategory)) {
          console.log("AI missed and category is available for steal. Initiating steal opportunity for Player 1.");
          setFeedback("AI answered incorrectly! This category is available for steal. Answer the missed question to steal it.");
          setStealTurn("Player 1");
        } else {
          console.log("Category not available for steal. Player 1 must choose a new category for steal.");
