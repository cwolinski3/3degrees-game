...
function App() {
  // Game phase: pregame and playing.
  ...
  const [currentTopic, setCurrentTopic] = useState(null); // Fixed topic once category is selected.
  ...
  // ----------------------------
  // Pregame: Player 1 selects 3 weaknesses for the AI.
  // ----------------------------
  ...
  // ----------------------------
  // Pregame: AI auto-selects its strengths and weaknesses for Player 1.
  // ----------------------------
  useEffect(() => {
  ...
  const handleSelectCategory = (cat) => {
    ...
    for (let i = 0; i < topics.length; i++) {
      ...
      console.log(`Topic candidate: ${candidate.topic}, Degree 1:`, candidate.degrees?.[1]);
  ...
  const handleSubmitAnswer = (providedAnswer) => {
    ...
    if (currentPlayer === "Player 1") {
      if (currentQuestion) {
        if (currentQuestion.type === "fill") {
          ...
          // For MC, if answer is an array, check if any match.
    ...
    if (isCorrect) {
      ...
      // BONUS: Check against the playing player's own weaknesses.
      let bonus = 0;
      if (currentPlayer === "Player 1") {
      ...
      basePoints += bonus;
      console.log(`Points for ${currentPlayer}: base=${degree*10}, bonus=${bonus}, total=${basePoints}`);
      setScore(prev => {
      ...
      // Steal mechanism: 60% chance (for both players)
      if (Math.random() < 0.6) {
        console.log(`${opponent} steals the question!`);
        ...
        console.log(`Steal points for ${opponent}: base=${degree*10}, bonus=${bonusSteal}, total=${stealPoints}`);
        setScore(prev => {
