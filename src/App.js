...
function App() {
  // Game phases: pregame and playing.
  ...
  const [currentTopic, setCurrentTopic] = useState(null); // Fixed topic for selected category.
  ...
  const handleSubmitAnswer = (providedAnswer) => {
    ...
    if (isCorrect) {
      ...
      // Bonus is now based on the current player's own weaknesses.
      let bonus = 0;
      if (currentPlayer === "Player 1") {
      ...
      // Determine opponent's available categories.
      let opponentAvailable;
      if (opponent === "Player 1") {
        opponentAvailable = [...player1Strengths, ...aiWeaknesses];
      } else {
        opponentAvailable = [...aiStrengths, ...player1Weaknesses];
      }
      const opponentCanAnswer = opponentAvailable.includes(selectedCategory);
      
      // Simulate opponent's steal answer.
      ...
      if (opponentCanAnswer && opponentAnswerCorrect) {
        ...
        basePoints += bonus;
        console.log(`Steal correct: ${opponent} gets base=${degree*10}, bonus=${bonus}, total=${basePoints}`);
        setScore(prev => {
        ...
        // Opponent cannot answer or answers incorrectly: Award steal points.
                ...
                currentQuestion.type === "mc" ? (
                  <>
                    {currentQuestion.choices.map((choice, index) => (
                      <button
                        key={index}
                        onClick={() => { console.log("MC choice selected:", choice); handleSubmitAnswer(choice); }}
                      >
                        {choice}
                      </button>
                    ))}
