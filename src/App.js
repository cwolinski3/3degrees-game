...
function App() {
  ...
  const [currentTopic, setCurrentTopic] = useState(null); // The topic fixed for the current turn.
  ...
  // --- Steal state ---
  // When non-null, indicates that a steal opportunity is active.
  // Only off-turn (Player 1) can initiate a steal.
  const [stealTurn, setStealTurn] = useState(null);

  // Determine the "active" player.
  // If a steal is active, the active player is forced to be the off-turn player (Player 1).
  ...
  // --- Pregame Setup: Player 1 selects 3 weaknesses for AI ---
  ...
  if (gameState === "playing") {
    if (activePlayer === "Player 1") {
      // For Player 1, available categories are those in their strengths or AI's weaknesses that they haven't mastered.
  ...
  // --- handleSelectCategory: When a category is chosen, clear previous topic state and pick a new random topic.
  const handleSelectCategory = (cat) => {
    console.log("handleSelectCategory called with:", cat);
    clearTurnState(); // Ensure we clear any previous topic.
  ...
  // If stealTurn is active, process the steal attempt with double points for that question.
  const handleSubmitAnswer = (providedAnswer) => {
    const userAnswer = providedAnswer !== undefined ? providedAnswer : answer;
    let isCorrect = false;
    
    if (stealTurn) {
      // Steal opportunity processing for Player 1.
      ...
      if (isCorrect) {
        ...
        // For a steal opportunity, award double points for that question.
        let basePoints = degree * 10;
        if (currentQuestion.type === "fill") basePoints *= 2;
        // For Player 1, if the category is a weakness, bonus = 10; if it's a strength, bonus = 0.
        const bonus = player1Weaknesses.includes(selectedCategory) ? 10 : 0;
        const stealPoints = 2 * (basePoints + bonus);
        console.log(`Steal correct: Player 1 earns double points = ${stealPoints}`);
        setScore(prev => ({ ...prev, ["Player 1"]: prev["Player 1"] + stealPoints }));
        // On a successful steal, Player 1 continues in the category.
        setStealTurn(null);
        if (degree < 3) {
          setDegree(degree + 1);
        } else {
    ...
    if (activePlayer === "Player 1") {
      ...
      // For AI, simulate correct answer with 70% chance.
    ...
    if (isCorrect) {
      ...
      let basePoints = degree * 10;
      if (currentQuestion.type === "fill") basePoints *= 2;
      // For normal turns, bonus is determined by whether the category is a weakness.
      // For Player 1: if category is in player1Weaknesses then bonus = 10, else bonus = 0.
      // For AI: if category is in aiWeaknesses then bonus = 10, else bonus = 0.
      ...
      console.log(`Correct: ${activePlayer} earns base=${basePoints}, bonus=${bonus}, total=${totalPoints}`);
      ...
      // If active player is AI, then check if the category is available for Player 1 to steal.
      if (activePlayer === "AI Player") {
        ...
        if (player1Available.includes(selectedCategory)) {
          ...
          setFeedback("AI answered incorrectly! This category is available for steal. Answer the missed question below.");
          // The off-turn player (Player 1) gets to attempt the same question (steal opportunity).
          setStealTurn("Player 1");
        } else {
          console.log("Category not available for steal. Player 1 must choose a new category to steal from.");
        ...
        if (oppCorrect) {
          ...
          console.log(`Steal correct: AI earns total=${stealPoints}`);
    ...
    <div style={{ textAlign: 'center', fontFamily: 'Arial' }}>
      ...
      {gameState === "playing" && (
        <>
          ...
          {stealTurn === "Player 1" && (
            <div>
              <h3>Steal Opportunity! (This category is available to steal)</h3>
              <h4>Select a Category for your steal attempt:</h4>
