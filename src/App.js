...
function App() {
  ...
  // --- Steal state ---
  // When non-null, forces a steal opportunity (only off-turn Player 1 can steal).
  ...
  // --- Pregame: Player 1 selects 3 weaknesses ---
  ...
  // --- Pregame: AI auto-selects strengths and weaknesses ---
  useEffect(() => {
  ...
  // --- handleStealCategory: For off-turn Player 1 to select a category for the steal attempt ---
  ...
  // For steal turns, process the off-turn player's steal attempt.
  const handleSubmitAnswer = (providedAnswer) => {
    ...
    if (stealTurn) {
      ...
      if (isCorrect) {
        ...
        // For steal, bonus is determined solely by Player 1's weakness: +10 if category is in their weakness.
        ...
        if (degree < 3) {
          ...
          clearTurnState();
          // Player 1 retains turn; they choose a new available category.
        }
      } else {
