...
function App() {
  ...
  const [currentTopic, setCurrentTopic] = useState(null); // Once chosen, this topic remains fixed.
  ...
  // New state: stealTurn forces control for a steal attempt.
  // When set to "Player 1", it means that even if turn parity says otherwise,
  // the steal opportunity belongs to Player 1.
  const [stealTurn, setStealTurn] = useState(null);

  // Determine current player: if stealTurn is active, use that; otherwise use turn % 2.
  ...
  if (gameState === "playing") {
    // For Player 1, available = Player1Strengths U aiWeaknesses; for AI, available = aiStrengths U player1Weaknesses.
  ...
  // ----------------------------
  // handleStealCategory: For the off-turn player to choose a category when stealing.
  // ----------------------------
  const handleStealCategory = (cat) => {
    ...
    setCurrentTopic(chosenTopic);
    setCurrentQuestion(q);
    setFeedback("Steal attempt: Answer the new question.");
    // Clear the stealTurn flag so that normal play resumes after the steal attempt.
    setStealTurn(null);
  ...
  // If not in steal mode, process normally. If in steal mode, treat it as the steal attempt.
  // ----------------------------
  const handleSubmitAnswer = (providedAnswer) => {
    const userAnswer = providedAnswer !== undefined ? providedAnswer : answer;
    let isCorrect = false;

    // If we are in a steal turn, then currentPlayer is already forced (e.g., "Player 1")
    if (stealTurn) {
      // Evaluate the steal attempt (only applicable when stealTurn is "Player 1")
      ...
      if (isCorrect) {
        ...
        // Successful steal: continue in this category.
        if (degree < 3) {
          setDegree(degree + 1);
        } else {
          ...
          // End steal phase and switch turn.
          setSelectedCategory(null);
          setCurrentTopic(null);
          setTurn(turn + 1);
          setRound(round + 1);
    ...
    if (currentPlayer === "Player 1") {
      ...
      // For AI's turn, simulate answer with 70% chance of being correct.
    ...
    if (isCorrect) {
      ...
      // If active player is AI, then trigger a steal opportunity for Player 1.
      if (currentPlayer === "AI Player") {
        ...
        // Do not increment turn here; wait for steal attempt.
      } else {
        // If Player 1 (active) answered incorrectly, simulate AI steal automatically.
        console.log("Player 1 missed. AI will attempt to steal automatically.");
    ...
    <div style={{ textAlign: 'center', fontFamily: 'Arial' }}>
      ...
      {gameState === "playing" && (
        <>
          ...
          {/* If a steal opportunity is active for Player 1, show steal category selection */}
          {stealTurn === "Player 1" && (
            <div>
          ...
          {/* Normal turn: if no category selected, show category selection */}
