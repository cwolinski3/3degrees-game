...
function App() {
  ...
  // currentTopic stores the chosen topic for the selected category
  ...
  // getQuestion: Uses currentTopic to retrieve a question for a given degree.
  ...
  // When a category is selected, try to find a topic within that category that has a non-empty array for Degree 1.
  const handleSelectCategory = (cat) => {
    ...
    let q = null;
    // Loop through topics to find one with a valid Degree 1 question array.
    ...
    if (!q) {
      setFeedback(`No questions available for ${cat} at Degree 1 in any topic. Please select a different category.`);
      setSelectedCategory(null);
  ...
  // useEffect to update the question when currentTopic or degree changes.
  useEffect(() => {
  ...
  // Game Over Condition: Only check during "playing" state.
  if (gameState === "playing") {
