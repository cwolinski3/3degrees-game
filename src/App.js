...
function App() {
  ...
  // currentTopic will store the chosen topic for the selected category
  ...
  // ----------------------------
  // getQuestion: Uses the fixed currentTopic to retrieve a question for a given degree.
  // ----------------------------
  ...
  // When a category is selected, choose one random topic from that category and store it.
  const handleSelectCategory = (cat) => {
    ...
    const q = chosenTopic.degrees[1]?.[Math.floor(Math.random() * chosenTopic.degrees[1].length)] || null;
  ...
  // ----------------------------
  // useEffect to fetch a new question when currentTopic or degree changes.
  // This ensures the question is pulled after state updates.
  // ----------------------------
  useEffect(() => {
    ...
    <div style={{ textAlign: 'center', fontFamily: 'Arial' }}>
      ...
      {gameState === "playing" && (
        <>
          ...
          <p>
            Score - Player 1: {score["Player 1"]} | AI Player: {score["AI Player"]}
          </p>
          <p>
            Mastered Categories - Player 1: {masteredCategories["Player 1"].join(", ")}
          </p>
          <p>
            Mastered Categories - AI Player: {masteredCategories["AI Player"].join(", ")}
          </p>
        </>
      )}
    </div>
